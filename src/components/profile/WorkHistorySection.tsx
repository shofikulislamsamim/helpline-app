...
import { Briefcase, Plus, Trash2, Calendar, MapPin, Building, CheckCircle2 } from 'lucide-react';
...
export const WorkHistorySection: React.FC<WorkHistorySectionProps> = ({
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    const newRecord: WorkHistory = {
      ...
      startDate: startDate.trim() || '২০২২',
      endDate: currentlyWorking ? 'চলমান' : (endDate.trim() || '২০২৪'),
      currentlyWorking,
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h4 className="font-bold text-xs sm:text-sm text-slate-900 flex items-center gap-2">
            ...
            <span>পূর্ববর্তী কর্মসংস্থান ও কাজের ইতিহাস (Work History)</span>
          </h4>
      ...
      {isAdding && (
        <form onSubmit={handleAdd} className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3 animate-in fade-in">
          <div className="flex items-center justify-between pb-2 border-b border-slate-200">
            <span className="text-xs font-bold text-slate-800">নতুন কাজের অভিজ্ঞতা যোগ</span>
            <button
              type="button"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-semibold text-slate-700 mb-1">
                প্রতিষ্ঠান বা মালিকের নাম (Company / Employer) <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                required
                ...
                placeholder="যেমন: ইস্টার্ন ইঞ্জিনিয়ারিং"
            ...
            <div>
              <label className="block text-[11px] font-semibold text-slate-700 mb-1">
                পদবী (Position / Designation) <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                required
                ...
                placeholder="যেমন: সিনিয়র ইলেকট্রিশিয়ান"
                className="w-full text-xs p-2 rounded-lg border border-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            <div>
              <label className="block text-[11px] font-semibold text-slate-700 mb-1">
                শুরুর সময় (Start Year/Date)
              </label>
              <input
                type="text"
                value={startDate}
            ...
            <div>
              <label className="block text-[11px] font-semibold text-slate-700 mb-1">
                শেষের সময় (End Year/Date)
              </label>
              <input
                type="text"
                disabled={currentlyWorking}
                value={currentlyWorking ? 'চলমান' : endDate}
          ...
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="currentlyWorkingCheck" className="text-xs text-slate-700 cursor-pointer font-medium">
              আমি বর্তমানে এখানে কর্মরত
            </label>
          </div>

          <div>
            <label className="block text-[11px] font-semibold text-slate-700 mb-1">
              কাজের এলাকা বা অবস্থান (Location)
            </label>
            <input
              type="text"
              value={location}
              onChange={(e) => setLocation(e.target.value)}
              placeholder="যেমন: গুলশান, ঢাকা"
              className="w-full text-xs p-2 rounded-lg border border-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
          ...
          <div>
            <label className="block text-[11px] font-semibold text-slate-700 mb-1">
              দায়িত্ব ও কাজের সংক্ষিপ্ত বিবরণ (Job Details)
            </label>
            <textarea
              rows={2}
              value={jobDetails}
              onChange={(e) => setJobDetails(e.target.value)}
              placeholder="এই প্রতিষ্ঠানে আপনার প্রধান কাজের দায়িত্ব কী ছিল..."
              className="w-full text-xs p-2 rounded-lg border border-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
      ...
      {histories.length > 0 ? (
        ...
        <div className="p-4 bg-slate-50 border border-dashed border-slate-200 rounded-xl text-center text-xs text-slate-500">
          এখনও কোনো পূর্ববর্তী কর্মসংস্থান বা কাজের ইতিহাস যুক্ত করা হয়নি।
        </div>
      )}
    </div>
  );
